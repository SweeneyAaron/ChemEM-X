...
from chimerax.ChemEM.simulation import (
    ...
    HbondAngleForce,
    PiPiDistForce,
    get_position_vector_from_atom,
    get_model_from_complex_structure,
    get_model_from_complex_structure_old,
    SSE_force,
    SSERigidBodyBBConstraint,
    HelixHbondForce,
    PsiAngleForce,
    PhiAnglelForce,
    AnchorAtoms,
    get_mmpbsa_complex_system,
    CationPiForce,
    SaltBridgeForce,
    WeakHbondDistForce,
    WeakHbondAngleForce,
...
from chimerax.ChemEM.binding_site.commands import SaveSite, AutoSiteFinder, BindingSiteFromMarker, ResetBindingSiteFields, AddBindingSiteFromInputsOrChange, RenderBindingSiteFromClick, IncrementBindingSiteCounter, AddBindingSite, AddBindingSiteToConf,  RemoveBindingSite, RemoveBindingSiteFromConf, TransferSiteToConf
from chimerax.ChemEM.binding_site.protocols import RenderBindingSite, AutoSiteChimera
#Delete below once commands is implimented!!
#from chimerax.ChemEM.map_masks.protocols import ConfidenceMap, BindingSiteMask, SignificantFeatures
#from chimerax.ChemEM.map_masks.commands import  RunSignificantFeatures, RunAutoMask
#from chimerax.ChemEM.map_masks.parameters import MaskOptionsParameter
...
class CHEMEM(HtmlToolInstance):
    ...
    def get_platforms(self):
        ...
        for i in range(Platform.getNumPlatforms()):
            ...
            if plt not in avalible_platforms:
    ...
    def remove_task(self, *args):
        ...
        if job.job_type == CHEMEM_JOB:
            ...
            if 'current_map' in self.current_simulation.parameters:
                ...
                if 'map_bias' in self.current_simulation.parameters:
                    map_bias = self.current_simulation.parameters['map_bias']
                else:
                    map_bias = False
            else:
                current_map = None
           ...
           # simulation = Simulation(self.session, job.params.output, current_map, platform_name = self.platform)
            #restraints for thing 

            if 'constrain_backbone' in self.current_simulation.parameters and self.current_simulation.parameters['constrain_backbone']:
                
                constrain_backbone = True 
            else:
                constrain_backbone = False
                
            
            if 'constrain_sidechain' in self.current_simulation.parameters and self.current_simulation.parameters['constrain_sidechain']:
                
                constrain_sidechain = True 
            else:
                constrain_sidechain = False
            
            
            if 'start_temp' in self.current_simulation.parameters:
                start_temp = self.current_simulation.parameters['start_temp'].value
            ...
            '''
            if 'constrain_protein' in self.current_simulation.parameters and self.current_simulation.parameters['constrain_protein']:
            ...
            #
            simulation.temperature = start_temp
            ...
            '''
            if constrain_protein and not constrain_backbone:
                simulation.add_force(ConstrainProtein) 
                restraint_id = len(self.current_restraints)
                message = 'Protein All Heavy Atoms'
                js_code = f'addRestraintToList("{message}", "{restraint_id}", "ConstrainProtein");'
                self.run_js_code(js_code)
            '''
            
            if constrain_backbone:
                simulation.add_force(ConstrainBackBone) 
                restraint_id = len(self.current_restraints)
                message = 'Protein BackBone Heavy Atoms'
                js_code = f'addRestraintToList("{message}", "{restraint_id}", "ConstrainBackBone");'
                self.run_js_code(js_code)
            
            if constrain_sidechain:
                simulation.add_force(ConstrainSideChain) 
                restraint_id = len(self.current_restraints)
            ...
            simulation.add_force(SaltBridgeForce)
            simulation.add_force(HbondDistForce)
            simulation.add_force(HbondAngleForce)
            simulation.add_force(WeakHbondDistForce)
            simulation.add_force(WeakHbondAngleForce)
            simulation.add_force(HalogenBondForce)
            
            
            simulation.add_force(PiPiDistForce)
            simulation.add_force(CationPiForce)
            
            simulation.set_simulation()
            
            #if map_bias:
            #    simulation.simulation.context.setParameter('global_k', map_bias.value)
            ...
            '''
            Add a new ligand to the simulation
            ...
            complex_structure.residues[-1].name = 'LIG'
            
            
            #TODO!!! if solvent!!! 
            #move the atoms now!!!
            #simulation_model, atoms_to_position_index = get_model_from_complex_structure_old(complex_structure, self.simulation_model)
            atoms_to_position_index = self.atoms_to_position_index
            for atom in self.simulation_model.residues[-1].atoms:
                name = atom.name 
                for atom2 in complex_structure.residues[-1].atoms:
                    if atom2.name == name:
                        atoms_to_position_index.append([atom, atom2.idx])
            ...
            self.simulation = simulation
            #-----
            simulation.set_ring_groups(self.simulation_model, self.atoms_to_position_index_as_dic)
            simulation.set_SSE_elements(self.simulation_model, self.atoms_to_position_index_as_dic)
            simulation.set_force_groups()
            ...
            if selected_atoms:
                simulation.set_anchor_residues(self.simulation_model, 
                                               self.current_simulation.parameters['selected_atom_data'],
                                               self.atoms_to_position_index_as_dic)
            ...
            '''
            Add a new covelent ligand to the simulation
            '''
    ...
    def run(cls, chemem, query):
        ...
        if chemem.rendered_site is not None and chemem.rendered_site.map is not None:
            if chemem.parameters.get_parameter('confidence_map') is None:
                ...
                confidence_map, confidence_map_volume = ConfidenceMap(chemem.session, chemem.rendered_site.map).run()
                
                chemem.parameters.add(confidence_map)
            
            confidence_map = chemem.parameters.get_parameter('confidence_map')
            binding_site_mask = BindingSiteMask(chemem.session,chemem.rendered_site, confidence_map=confidence_map_volume )
...
class SetCationPiTTug(Command):
    ...
    def js_code(cls, ring_atom, cation_atom, restraint_id):
        ...
        ring = escape_js_string(trim_pdb_info(ring_atom.residue.string()))
        
        cation = escape_js_string(trim_pdb_info(cation_atom.string()))
    
        message = f'Ring: {ring}\\nCation: {cation}'
        
        # Create the JavaScript code string safely
        js_code = f'addRestraintToList("{message}", "{restraint_id}", "CationPiTug");'
        return js_code
    ...
    def run(cls, chemem, query):
        if chemem.current_simualtion_id is not None:
            ...
            ring_atoms, ring_atom_indices, cation_atom, cation_atom_index = get_cation_pi_tug_indexes(simulation_model, chemem.atoms_to_position_index_as_dic)
            job.cation_pi_tug = [ring_atom_indices, cation_atom_index, None, None, None]
            
            restraint_id = len(chemem.current_restraints)
            chemem.current_restraints[restraint_id] = [[ring_atom_indices, cation_atom_index],[ring_atoms[0],cation_atom]]
            js_code = cls.js_code(ring_atoms[0], cation_atom, restraint_id)
            chemem.run_js_code(js_code)
            com = f'distance {ring_atoms[0].atomspec} {cation_atom.atomspec}'
...
class SetPiPiTTug(Command):
    ...
    def js_code(cls, selected_atoms, restraint_id):
        ...
        # Construct the message with all special characters escaped
        ring_1 = escape_js_string(trim_pdb_info(selected_atoms[0].residue.string()))
        
        ring_2 = escape_js_string(trim_pdb_info(selected_atoms[1].residue.string()))
    
        message = f'Ring 1: {ring_1}\\nRing 2: {ring_2}'
        
        # Create the JavaScript code string safely
        js_code = f'addRestraintToList("{message}", "{restraint_id}", "PiPiTug");'
        return js_code
    ...
    def run(cls, chemem, query ):
        if chemem.current_simualtion_id is not None:
            ...
            # make this a centroid tracker
            ring_atoms = [i[0] for i in atoms_index_pairs]
            ring_atoms = [i[0] for i in ring_atoms]
            chemem.current_restraints[restraint_id] = [openff_index_pairs, ring_atoms]
            
            js_code = cls.js_code(ring_atoms, restraint_id)
            chemem.run_js_code(js_code)
            #add the fucntion to total restraints!
            com = f'distance {ring_atoms[0].atomspec} {ring_atoms[1].atomspec}'
            run(chemem.session, com)
...
class SetSaltBridgeTug(Command):
    ...
    def js_code(cls, selected_atoms, restraint_id):
        ...
        # Construct the message with all special characters escaped
        cation = escape_js_string(trim_pdb_info(selected_atoms[0].string()))
        anion = escape_js_string(trim_pdb_info(selected_atoms[1].string()))
        
    
        message = f'Salt Bridge: {cation} - {anion}'
    
        # Create the JavaScript code string safely
        js_code = f'addRestraintToList("{message}", "{restraint_id}", "SaltBridgeTug");'
        return js_code
    ...
    def run(cls,chemem,query):
        if chemem.current_simualtion_id is not None:
            ...
            job = chemem.job_handeler.jobs[simulation_id]
            simulation_model = chemem.simulation_model
            saltbridge_idx, selected_atoms = get_saltbridge_tug_index(simulation_model, chemem.atoms_to_position_index_as_dic)
            
            print('SALTBRIDGE')
            print(saltbridge_idx)
            print(selected_atoms)
            
            if None in selected_atoms:
                return
            ...
            com = f'distance {selected_atoms[0].atomspec} {selected_atoms[1].atomspec}'
            run(chemem.session, com)
            chemem.current_restraints[restraint_id] = [saltbridge_idx, selected_atoms]
...
class SetWeakHBondTug(Command):
    ...
    def js_code(cls, selected_atoms, restraint_id):
        ...
        # Create the JavaScript code string safely
        js_code = f'addRestraintToList("{message}", "{restraint_id}", "WeakHbondTug");'
        return js_code
    ...
    def run(cls,chemem,query):
        if chemem.current_simualtion_id is not None:
            ...
            weak_hbond_idx, selected_atoms = get_weak_hbond_tug_index(simulation_model, chemem.atoms_to_position_index_as_dic)
            ...
            com = 'distance sel'
            run(chemem.session, com)
            chemem.current_restraints[restraint_id] = [weak_hbond_idx, selected_atoms]
...
class SetHBondTug(Command):
    ...
    def js_code(cls, selected_atoms, restraint_id):
        ...
        # Create the JavaScript code string safely
        js_code = f'addRestraintToList("{message}", "{restraint_id}", "HbondTug");'
        return js_code
...
class SetHalogenBondTug(Command):
    ...
    def js_code(cls, selected_atoms, restraint_id):
        ...
        # Construct the message with all special characters escaped
        donor = escape_js_string(trim_pdb_info(selected_atoms[0].string()))
        acceptor = escape_js_string(trim_pdb_info(selected_atoms[1].string()))
    
        message = f'Halogen Bond Donor: {donor}\\nAcceptor: {acceptor}'
    
        # Create the JavaScript code string safely
        js_code = f'addRestraintToList("{message}", "{restraint_id}", "HalogenBondTug");'
        return js_code
    ...
    def run(cls, chemem, query):
        if chemem.current_simualtion_id is not None:
            simulation_id = chemem.current_simualtion_id
            job = chemem.job_handeler.jobs[simulation_id]
            indices, selected_atoms, theta2_0, halogen_distance = get_halogen_bond_tug_index(chemem.simulation_model,  chemem.atoms_to_position_index_as_dic)
            if indices is None:
                print('Halogen Bond not found!')
                return 
            job.halogen_bond_tug = [indices, None, None,None,halogen_distance * unit.angstrom, None, theta2_0 * unit.degrees]
            restraint_id = len(chemem.current_restraints)
            ...
            com = 'distance sel'
            run(chemem.session, com)
            chemem.current_restraints[restraint_id] = [indices, selected_atoms, theta2_0, halogen_distance]
...
class DeleteConstrainProtein(Command):
    ...
    def run(cls, chemem, query):
        if chemem.current_simualtion_id is not None:
            chemem.simulation.simulation.context.setParameter('backbone_constraint_k', 0)
            chemem.simulation.simulation.context.setParameter('sidechain_constraint_k', 0)
...
class DeleteHalogenBondTug(Command):
    ...
    def run(cls, chemem, query):
        if chemem.current_simualtion_id is not None:
            simulation_id = chemem.current_simualtion_id
            job = chemem.job_handeler.jobs[simulation_id]
            indices, selected_atoms, theta2_0, halogen_distance = chemem.current_restraints[int(query)]
            job.halogen_bond_tug = [indices, 0.0, 0.0, 0.0, halogen_distance * unit.angstrom, None, theta2_0 * unit.degrees]
            com = f'distance delete {selected_atoms[0].atomspec} {selected_atoms[1].atomspec}'
            run(chemem.session, com)
...
class DeletePiPiTug(Command):
    ...
    def run(cls, chemem, query):
        if chemem.current_simualtion_id is not None:
            simulation_id = chemem.current_simualtion_id
            job = chemem.job_handeler.jobs[simulation_id]
            openff_ring_atoms , selected_atoms = chemem.current_restraints[int(query)]
            com = f'distance delete {selected_atoms[0].atomspec} {selected_atoms[1].atomspec}'
            run(chemem.session, com)
...
class DeleteCationPiTug(Command):
    ...
    def run(cls, chemem, query):
        if chemem.current_simualtion_id is not None:
            simulation_id = chemem.current_simualtion_id
            job = chemem.job_handeler.jobs[simulation_id]
            indices, distance_atoms = chemem.current_restraints[int(query)]
            job.cation_pi_tug = [indices[0], indices[1], 0.0, 0.0, 0.0]
            com = f'distance delete {distance_atoms[0].atomspec} {distance_atoms[1].atomspec}'
            run(chemem.session, com)
            del chemem.current_restraints[int(query)]
...
class DeleteSaltBridgeTug(Command):
    ...
    def run(cls, chemem, query):
        if chemem.current_simualtion_id is not None:
            simulation_id = chemem.current_simualtion_id
            job = chemem.job_handeler.jobs[simulation_id]
            indices, selected_atoms = chemem.current_restraints[int(query)]
            job.saltbridge_tug = [indices, None]
            com = f'distance delete {selected_atoms[0].atomspec} {selected_atoms[1].atomspec}'
            run(chemem.session, com)
...
def get_cation_pi_tug_indexes(model, atoms_to_index):
    ...
    # Get all selected atoms in the model
    #Needs testing and error handeling
    ...
    # Separate selected atoms into protein and ligand atoms
    protein_atoms = [atom for atom in selected_atoms if atom.residue.name in RD_PROTEIN_SMILES]
    ...
    if protein_rings:
        ...
        if cation_atom is None:
            ...
            return None, None, None, None  # No cation found in ligand
        ...
        if cation_atom is None:
            ...
            return None, None, None, None  # No cation found in protein
    else:
        
        return None, None, None, None  # No aromatic rings found in selection
    ...
    if cation_atom_index is None:
        ...
        return None, None, None, None  # Cation atom not found in atoms_to_index mapping
    
    return ring_atoms, ring_atom_indices, cation_atom, cation_atom_index
    
def find_cation_atom(atoms_list):
    # Search for cationic atoms in the given list of atoms
    for atom in atoms_list:
        residue = atom.residue
        # Check for common cationic residues in proteins
        if residue.name == 'LYS' and atom.name == 'NZ':
            ...
            # Histidine can be positively charged at certain pH levels
            return atom
        # For ligands, check for nitrogen atoms with positive formal charge
        if atom.element.name == 'N' and atom.formal_charge == 1:
            return atom
        # Check for quaternary ammonium groups in ligands
        if atom.element.name == 'N' and len(atom.bonds) == 4:
            return atom
    return None  # No cationic atom found
...
def get_halogen_bond_tug_index(model, atoms_to_index):
    ...
    #get bond atoms 
    bonds_with_halogen = halogen.bonds 
    atoms_bonded_to_halogen = []
    for bond in bonds_with_halogen:
        atoms = bond.atoms 
        for a in atoms:
            if a != halogen:
    ...
    bonds_with_acceptor = acceptor.bonds 
    
    atoms_bonded_to_acceptor = []
    for bond in bonds_with_acceptor:
        atoms = bond.atoms
        for a in atoms:
            if a != acceptor:
    ...
    #assign hybrisation state
    if element == 'O':
        if n_bonded_atoms == 1:
            hybridization = 'sp3'
            theta2_0 = 109.5  # Degrees
        elif n_bonded_atoms == 2:
            hybridization = 'sp2'
            theta2_0 = 120.0  # Degrees
        else:
            # Unusual bonding environment
            return None, None, None, None
    elif element == 'N':
        if n_bonded_atoms == 2:
            hybridization = 'sp2'
            ...
            hybridization = 'sp'   # For triple bonds
            theta2_0 = 180.0       # Degrees
        else:
            return None, None, None, None
    elif element == 'S':
        # Sulfur can have various hybridizations; here we consider common cases
        if n_bonded_atoms == 2:
            hybridization = 'sp3'
            theta2_0 = 109.5  # Degrees
        elif n_bonded_atoms == 1:
